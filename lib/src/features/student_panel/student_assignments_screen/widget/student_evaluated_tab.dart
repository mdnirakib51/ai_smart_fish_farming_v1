...
import 'student_assignments_widget.dart';

class StudentEvaluatedTab extends StatefulWidget {
  const StudentEvaluatedTab({
    super.key,
  });

  @override
  State<StudentEvaluatedTab> createState() => _StudentEvaluatedTabState();
}

class _StudentEvaluatedTabState extends State<StudentEvaluatedTab> {
  ...
  void handleFileSelection(int index, PlatformFile? file) {
    ...
    if (file != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('File "${file.name}" selected for Assignment ${index + 1}'),
  ...
  Future<void> submitHomework(int index) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Assignment ${index + 1} submitted successfully!'),
        backgroundColor: Colors.green,
      ),
    );
  }
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (ctx, index) {
          return StudentAssignmentsWidget(
            subject: 'Math',
            code: '101',
